...
async function registerUser(req, res) {
  try {
    ...
    const userId = await userService.registerUser(name, username, password);
...
async function getRegiteredUsersList(req, res) {
  try {
    const usersList = await userService.getUsersList();
    return res.send({ message: "Success", usersList: usersList });
  } catch (err) {
    return res.status(500).send({ error: err });
  }
}

module.exports = { registerUser, loginUser, getRegiteredUsersList };
